                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-5 px-5 pt-3'>
                    <div className='md:col-span-2  bg-gray-100 p-8 rounded'>
                                ...
                                {visibleOrders.map((order) => (
                                    <li key={order.id} className='p-2'>
                                        <div className='p-3 border-b-2 md:flex-row justify-between items-center flex flex-col font-bold gap-3'>
                                            <img src={order.event.imgUrl} alt={order.event.name} className='md:w-[30%] w-[70%]' />
                                            <div className='flex flex-col justify-center p-5 items-center w-[20rem]'>
                                                <span>{order.event.name}</span>
                                                <span>{formatDate(order.event.date)}</span>
                                                <span>{order.isUsed ? <span className='text-[#56c95a]'>USADO</span> : <span className='text-[#de4545]'>SIN USAR</span> }</span>
                        ...
                        <div className='flex justify-center my-4'>
                            <button onClick={prev} disabled={page === 1} className="mr-2 px-4 py-2 bg-gray-200  hover:scale-105 transition-transform">Anterior</button>
                            <button onClick={next} disabled={page === totalPages} className="mr-2 px-4 py-2 bg-gray-200  hover:scale-105 transition-transform">Siguiente</button>
                        </div>
                    </div>
